...
- Nice defaults: using springs for css-properties involving motion and easings for others. (There's hardly ever a need to use a bouncy spring behaviour for e.g. `opacity`). The default animation speed seems is set way too slow for most of my animation needs however.
